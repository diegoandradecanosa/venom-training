        ...
        half *shared_mem_workspace)
{
    ...
    const half* B_panel = &B[idx_block_N*(block_N*mm/4)*K];//
    ...
    int A_warp_panel_offset = idx_warp_M * warp_M * block_K;
    ...
    for(; compute < num_tile; compute++) {
        ...
        for (int k = 0; k < block_K / mma_K; k++) {
            ...
            for (int m = 0; m < load_mma_M; m+=1) {
                int offset = A_warp_panel_offset + m*mma_M*2*block_K + k*mma_K;
                my_mma::load_matrix_sync2<ASwizzle>(afrag[0][m*2], shared_tile_A, offset, smem_lda);
                my_mma::load_matrix_sync2<ASwizzle>(afrag[0][m*2+1], shared_tile_A, offset+2*block_K, smem_lda);
            }
    ...
    for(int m=0; m<load_mma_M; m++){
        ...
        for(int n=0; n<iter_mma_N; n++){
            ...
            /* uint2 src[2];
            src[0] = my_mma::to_half4(*(float4*)&result[0]);
            src[1] = my_mma::to_half4(*(float4*)&result[4]);

            *(float4*)(C_panel + m*32*N_sp_p + n*16*mma_N/2) = *(float4*)(&src[0]); */
