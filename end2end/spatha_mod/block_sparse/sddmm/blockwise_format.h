...
struct BlockwiseSpTensor {
    ...
    void sync_host() {
        if (nnzb == 0) return;
        CUDA_CHECK( cudaMemcpy(this->values.data(), device_ref.values,
            this->nnzb*sizeof(T), cudaMemcpyDeviceToHost));
    }
...
template<typename T>
void BlockwiseSpTensor<T>::init_random(int nrow_, int ncol_, int brow_, int n_, int m_, int density_, unsigned seed_, int mbrow_, int bm_){
    ...
    BlockwiseSpTensor<T>::random_mask(nrow_, A_num_cols_sp, A_num_cols_sp_pad_nm, n_, m_, mbrow_, density, this->metadata, this->indices, seed_, bm_);
...
template<typename T>
void BlockwiseSpTensor<T>::random_mask(int nrow_, int ncol_, int ncol_pad, int n, int m, int mbrow, float density, std::vector<uint> &metadata, std::vector<uint> &indices, unsigned seed, int bm_)
{
...
template<typename T>
void BlockwiseSpTensor<T>::init_sparse(int nrow_, int ncol_, int brow_, int n_, int m_, int nnz_, const std::vector<unsigned int>& hA_metadata, std::vector<T>& hA_values, const std::vector<unsigned int>& hA_indices, unsigned seed_, int mbrow_, int bm_){
...
template<typename T>
void BlockwiseSpTensor<T>::init_sparse_device(int nrow_, int ncol_, int brow_, int n_, int m_, int nnz_, int* hA_metadata, T* hA_values, int* hA_indices, unsigned seed_, int mbrow_, int bm_){
...
template<typename T>
void BlockwiseSpTensor<T>::transform_and_sync_device() {
