...
struct SpmmBlockwiseKernel {
    ...
    void mainLoop(const int M, const int N, const int K,
        ...
        const half *A_values, const uint* A_indices, const half *B, half *shared_mem_workspace, float alpha, half *C, float beta, half *D);

    __device__ __forceinline__
    void epilogue(const int M, const int N,
        half *shared_mem_workspace, float alpha, half *C, float beta, half *D
    );
    ...
    const half *A_values, const uint *A_indices, const half *B, half *shared_mem_workspace, float alpha, half *C, float beta, half *D)
{
    ...
    const int iter_copy_C = CEIL(block_M * block_N / 8, blockDim);
    int global_offset = idx_block_M*block_M + idx_block_N*block_N*M + idx_warp_M*warp_M + idx_warp_N*warp_N*M + (lane%4)*2*M + (lane/4)*4;
    half *C_tile = C + global_offset;

/*
    half *D_tile = D+idx_block_M*block_M+idx_warp_M*warp_M+(lane%4)*2;
    __half2 bb = __float2half2_rn(alpha);
    __half2 aa = __float2half2_rn(beta);
    ...
    for (int i = 0; i < iter_copy_C; i++) {
        uint *dst  = (uint*) (C_tile+i*mma_N*M);
        uint *src2 = (uint*) (D_tile);//+i);

        src[0] = my_mma::to_half4(*(float4*)&cfrag[i][0].x[0]);
        src[1] = my_mma::to_half4(*(float4*)&cfrag[i][0].x[4]);

        *((float*)(&buffer2[0])) = *((float*)(src2));

        buffer[0]=buffer2[0].x; buffer[1]=buffer2[0].x;
        buffer[2]=buffer2[0].x; buffer[3]=buffer2[0].x;

        __half2 *dst3 = (__half2*)&src[0];
        __half2 *dst2 = (__half2*)&buffer[0];
        for (int i = 0; i < 2; i++) {
            dst3[i] = __hfma2(dst3[i], bb, dst2[i]);
        }
        *((float2*)(dst)) = *((float2*)&src[0]);


        buffer[0]=buffer2[0].y; buffer[1]=buffer2[0].y;
        buffer[2]=buffer2[0].y; buffer[3]=buffer2[0].y;
        dst3 = (__half2*)&src[1];
        for (int i = 0; i < 2; i++) {
            dst3[i] = __hfma2(dst3[i], bb, dst2[i]);
        }
        *((float2*)(dst+M/2)) = *((float2*)&src[1]);
    }
*/


    half *D_tile = D + idx_block_M*block_M + idx_warp_M*warp_M + (lane/4)*4; //+idx_block_N*block_N+idx_warp_N*warp_N+(lane%4)*2;
    __half2 bb = __float2half2_rn(alpha);
    __half2 aa = __float2half2_rn(beta);
    ...
    /* if(!blockIdx.x && !blockIdx.y && !threadIdx.x && !threadIdx.y){
        printf(" %.2f %.2f %.2f %.2f\n", cfrag[0][0].x[0], cfrag[0][0].x[1], cfrag[0][0].x[2], cfrag[0][0].x[3] );
    } */
    ...
    for (int i = 0; i < iter_copy_C; i++) {
        ...
        *((float2*)(&buffer2[0])) = *((float2*)(src2));
        //*((float2*)(&buffer[0])) = *((float2*)(src2));
        buffer[0]=buffer2[0].x; buffer[1]=buffer2[0].y;
        buffer[2]=buffer2[1].x; buffer[3]=buffer2[1].y;
...
(const int M, const int N,
    ...
    epilogue_impl<BM, block_N, blockDim, warp_M, CSwizzle, GridMappingXYToMN>(M, N, shared_mem_workspace, alpha, C, beta, D);
}
...
{
    epilogue_impl<BM, block_N, blockDim, warp_M, CSwizzle, GridMappingXYToMN>(M, N,  A_row_indices, D, shared_mem_workspace, alpha, C, beta);
}
