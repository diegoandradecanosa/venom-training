...
void load_matrix_sync2(fragment_a_rowmajor<ShapeBase<16, 16, 16>> &a,
    const half *base, const int offset, const int ldm, const int A_warp_panel_offset);
template<class F> DEVICE_INLINE
    ...
    const half *base, const int offset, const int ldm, const int A_warp_panel_offset)
{
    ...
    int row  = (lane%8)*4 + ((lane/8)%2);
    int col  = (lane/16)*8;
    int idx = offset + row*ldm + col;
    F f;
    //
    const half *src = base + A_warp_panel_offset + (row/16)*ldm + (idx/512)*512 + f(idx%512);
    ldmatrix<false, 4, 16>(a.x, src);
}
    ...
    const half *base, const int offset, const int ldm)
{
    ...
    int idx = offset + row*ldm + col;
    F f;
    const half *src = base + (idx/512)*512 + f(idx%512);
    ldmatrix<false, 4, 16>(b.x, src);
}
