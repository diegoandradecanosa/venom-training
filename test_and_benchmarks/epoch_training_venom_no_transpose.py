...
def train_epoch(model, train_loader, epoch, rank, profiler):
    ...
    print("Accumulated SPMM reshape time:", venom_times.spmm_reshape_time/1000000, "milliseconds.")# Accumulator gives nanoseconds, convert to milli
    print("Accumulated SDDMM reshape time:", venom_times.sddmm_reshape_time/1000000, "milliseconds.")# Accumulator gives nanoseconds, convert to milli
...
def load_distributed_model_and_train(num_repeats, number, rank, world_size):
    ...
    print("Model loaded and changing to venom kernels with no transposition using a "+str(v)+":"+str(n)+":"+str(m)+" mask.")
    venom_model = vsp.linear_to_venom_no_transposition(model, v, n, m)
    
    distributed_model = DDP(venom_model, device_ids=[rank], output_device=local_rank, find_unused_parameters=True)
    ...
    if args.profile:
        ...
        print('venom_no_transpose'+","+str(v)+","+str(n)+","+str(m)+","+str(bs)+","+str(total_time_nanoseconds/1000000)+","+str((total_time_nanoseconds-venom_times.sddmm_reshape_time)/1000000))
        ...
        #report_time('venom_no_transpose', v, n, m, bs, sparse_times, number)
