...
@cache
def group_n_m2(dense_shape, dense_dtype, n, m, v):
    nrows = dense_shape[0]
    ncols = dense_shape[1]
                        ...
                        for(int col_j=col_i+1; col_j<{m}; col_j++){{
                            ...
                            for(int col_k=col_j+1; col_k<{m}; col_k++){{
                                ...
                                for(int col_w=col_k+1; col_w<{m}; col_w++){{
                                    ...
                                    std::vector<int> mask({v}*{m}, 0);

                                    for(int row_i=0; row_i<{v}; row_i++){{
                ...
                if(remainder<3){{
                    ...
                    for(int bm_i=0; bm_i<bm_m; bm_i++){{
                        ...
                        for(int mcol_i=mcol_k; mcol_i<mcol_k_p; mcol_i++){{
                            ...
                            std::vector<int> masks_max({v}*remainder, 0);
                            ...
                            for(int col_i=0; col_i<remainder; col_i++){{
                                ...
                                for(int col_j=col_i+1; col_j<remainder; col_j++){{
                                    ...
                                    for(int col_k=col_j+1; col_k<remainder; col_k++){{
                                        vx[2]=col_k;
                                        total=0;
                                        std::vector<int> mask({v}*remainder, 0);
...
@cache
def to_dense(dense_shape, dense_dtype, n, m, v):
    nrows = dense_shape[0]
    ncols = dense_shape[1]
...
@cache
def to_sparse_venom(dense_shape, dense_dtype, n, m, v):
    nrows = dense_shape[0]
    ncols = dense_shape[1]
...
class VenomTensor:
    def __init__(self, n_, m_, v_, dense_, mask_, columns_, device_):
        ...
        self.nrows = None
        self.ncols = None
        #self.dense = dense_.to(device=device_)
        self.dense = dense_.cpu().to(dtype=torch.float32)
        self.device=device_
        #self.mask = mask_.to(device=device_)
        #self.mask = mask_

        ##self.masked = torch.nn.Parameter(dense_.to(device=self.device).half()*mask_.to(device=self.device).half())
        self.columns = columns_
        self.values = None
        self.metadata = None
        #self.pruned = dense_.to(device=device_)*mask_.half().to(device=device_)

        self.to_sparse_venom(dense_.cpu().to(dtype=torch.float32), mask_.cpu())
        #self.to_sparse_venom(dense_.to(dtype=torch.float32), mask_)
        #self.dense = None

    def to_sparse_venom(self, dense_, mask_):
        impl_builder = (
            to_sparse_venom
            )
        func = impl_builder(
        ...
        self.values = torch.zeros(self.nrows * A_num_cols_sp_pad, dtype=torch.float32, device="cpu") #dense_.dtype
        self.metadata = torch.zeros(self.nrows//mrow_m * A_num_cols_sp_pad//nelems_col, dtype=torch.int32, device="cpu")
        """ print("to_sparse_venom", dense_.dtype, mask_.dtype, self.values.dtype, self.columns.dtype, self.metadata.dtype)
        print("to_sparse_venom", dense_.device, mask_.device, self.values.device, self.columns.device, self.metadata.device) """
        func(dense_.data_ptr(), mask_.data_ptr(), self.values.data_ptr(), self.columns.data_ptr(), self.metadata.data_ptr())

        #self.dense = self.to_dense()
...
def venom_mask_sparsify(tensor, n, m, v):
    #print("venom_mask_sparsify", n, m, v)
    ...
    columns = torch.zeros(nrows//v * A_num_cols_sp_pad//n*m_fixed,dtype=torch.int32, device='cpu')
