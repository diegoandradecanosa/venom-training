...
class VENOMSparsifier:
    ...
    def __call__(self, tensor, grad_fmt=None, mask=None):
        ...
        if mask is None:
            mask = NMVectorSparsifier.get_static_mask(tensor, self.m, self.v)
...
class NMVectorSparsifier:
    ...
    @staticmethod
    def get_static_mask(tensor, m, v):
...
# Autograd function to perform forward and backwards operations using VENOM tensors and sparse computation from spatha
class VenomSparseLinearFunctionNoTransposition(torch.autograd.Function):
    @staticmethod
    def forward(ctx, 
                input, 
                sparse_weights_values, 
    ...
    def backward(ctx, grad_output):
        ...
        if ctx.needs_input_grad[1]: 
            sddmm_grad_output = torch.flatten(grad_output, start_dim=0, end_dim=-2)
            #sddmm_grad_output = sddmm_grad_output.T.contiguous()
            sddmm_grad_output = sddmm_grad_output.reshape(sddmm_grad_output.shape[1], sddmm_grad_output.shape[0]).contiguous()
            sddmm_input = torch.flatten(input, start_dim=0, end_dim=-2).half()
            #sddmm_input = sddmm_input.T.contiguous()# Transpose both since sddmm does A@B.T, but we want A.T@B.
            sddmm_input = sddmm_input.reshape(sddmm_input.shape[1], sddmm_input.shape[0]).contiguous()
        ...
        #forward(ctx,   input,      sparse_weights_values,  bias,       sparse_weights_columns, sparse_weights_metadata,    sparse_weights_dense,   venom_v,    venom_n,    venom_m,    nnz):
        return          grad_input, grad_weight,            grad_bias,  None,                   None,                       None,                   None,       None,       None,       None
    

# Torch.nn.Module subclass that sparsifies an existing torch.nn.lineal layer and uses dense algebra with a masked weights tensor.
class VenomSparseLinearNoTranspose(torch.nn.Module):
    def __init__(self, v, n, m, original: torch.nn.Linear = None, weights=None, bias=None, mask=None):
        super(VenomSparseLinearNoTranspose, self).__init__()        
...
def linear_to_venom_no_transposition(mod, v:int, n:int, m:int):
    """
    replaces linear modules for spmm sparse kernels from venom that make no tranposition (invalid operation results, but no overhead).
    """
    if isinstance(mod, torch.nn.Linear):
        return VenomSparseLinearNoTranspose(v, n, m, mod)

    for name, submod in mod.named_children():
        if isinstance(submod, VenomSparseLinearNoTranspose) or name=="classifier":
            continue
        if isinstance(submod, torch.nn.Linear):
            setattr(mod, name, VenomSparseLinearNoTranspose(v, n, m, submod))
